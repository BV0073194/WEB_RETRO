/**
 * Nostalgist.js - The main library script
 */
(function(global, factory) {
  ...
  class ResolvableFile {
    ...
    static async create(rawOrOption) {
      ...
      const option = typeof rawOrOption === "object" && "raw" in rawOrOption ? rawOrOption : {
        raw: rawOrOption
      };
  ...
  async function importCoreJsAsESM({
    js,
    name
  }) {
  ...
  const _EmulatorOptions = class _EmulatorOptions {
    ...
    async updateShader() {
      ...
      this.shader = await Promise.all(
        rawShaderFiles.map((rawShaderFile2) => ResolvableFile.create({
          raw: rawShaderFile2,
          signal: this.signal
        }))
      );
  ...
  class Emulator {
    constructor(options) {
      __publicField(this, "canvasInitialSize", {
        height: 0,
        width: 0
      });
    ...
    async saveState() {
      ...
      const thumbnail = stateThumbnailBuffer ? new Blob([stateThumbnailBuffer], {
        type: "image/png"
      }) : void 0;
      return {
        state,
...
`);
    ...
    postRun() {
      ...
      for (const gamepad of((_a = navigator.getGamepads) == null ? void 0 : _a.call(navigator)) ?? []) {
        if (gamepad) {
          globalThis.dispatchEvent(new GamepadEvent("gamepadconnected", {
            gamepad
          }));
        }
      }
    ...
    async setupEmscripten() {
      ...
      checkIsAborted(this.options.signal);
      const emscripten = await getEmscripten({
  ...
  const _Nostalgist = class _Nostalgist {
    ...
    static async launchSystem(system, options) {
      ...
      const launchOptions = isResolvableFileInput(optionsResult) ? {
        rom: optionsResult
      } : optionsResult;
      return await _Nostalgist.launch({ ...launchOptions,
...
/**
 * -------------------------------------------------------------------------
 * Custom Script for File Upload Functionality
 * -------------------------------------------------------------------------
 */
(function() {
  'use strict';
  ...
  // --- CONFIGURATION ---
  // Change this to the ID of the canvas element you want the emulator to use.
  const TARGET_CANVAS_ID = 'game-world';
  // -------------------

  /**
   * Creates the UI elements and sets up event listeners for file uploading.
   */
  function createUploader() {
    // Before doing anything, check if the target canvas exists in the document.
    const targetCanvas = document.getElementById(TARGET_CANVAS_ID);
    if (!targetCanvas) {
      console.error(`Nostalgist Error: Cannot find a canvas with the id "${TARGET_CANVAS_ID}".`);
      console.error('Please ensure a <canvas id="world"></canvas> element exists in your HTML.');
      alert(`Error: Canvas with ID "${TARGET_CANVAS_ID}" not found!`);
      return; // Stop the script if the canvas isn't found.
    }

    const fileInput = document.createElement('input');
    fileInput.type = 'file';
    fileInput.accept = '.nes,.smc';
    fileInput.style.display = 'none';

    const uploadButton = document.createElement('button');
    uploadButton.textContent = 'Click to upload and play a rom file!';
    ...
    fileInput.addEventListener('change', async (event) => {
      ...
      console.log(`File selected: ${romFile.name}`);

      try {
        uploadButton.style.display = 'none';

        console.log(`Launching Nostalgist on canvas element "#${TARGET_CANVAS_ID}"...`);
        // We pass an options object to Nostalgist.nes to specify the rom and the element.
        // The 'element' property accepts a CSS selector.
        if(romFile.name.includes('.nes')){
          await Nostalgist.nes({
              rom: romFile,
              element: `#${TARGET_CANVAS_ID}`
          });
        } else if(romFile.name.includes('.smc')){
        ...
        alert(`Error launching the emulator: ${error.message}`);
      }
    });
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', createUploader);
  } else {
    createUploader();
  }
})();